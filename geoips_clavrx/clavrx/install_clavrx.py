...
def get_env(netcdf_dir, hdf4_dir, hdf5_dir):
    ...
    environ["LIBRARY_PATH"] = (
        f"{netcdf_dir}/lib:{hdf4_dir}/lib:\
    {hdf5_dir}/lib:{environ.get('LIBRARY_PATH', '')}"
    )
...
def install_clavrx(
    ...
    logging.basicConfig(
        filename="clavrx_install_{}.log".format(
    ...
    gfortran_ver = ".".join(gfortran_path.split(".")[:1])

    if int(gfortran_ver[-1]) <= 9:
        run_shell_command(
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--hdf5_ver", type=str, default="1.10.9", help="hdf5 version e.g. 1.10.9"
    )

    parser.add_argument(
        "--netcdf_ver", type=str, default="4.9.0", help="netcdf4 version e.g. 4.9.0"
    )
